...
\begin{frame}[allowframebreaks]{Exemple : tag RFID}
...
\begin{columns}
  \begin{column}{0.45\textwidth}
	...
	\begin{itemize}
		\item un transpondeur ou radio-étiquette qui comporte un code permettant d'identifier un objet (ou autre)
		\item un émetteur-lecteur qui alimente le transpondeur et lit son code
	\end{itemize}  
  \end{column}
  \begin{column}{0.45\textwidth}
	\includegraphics[width=\textwidth]{Sistema-Rfid-Onidentity.jpeg}
  \end{column}
\end{columns}

\begin{block}{Illustration : \href{https://everycircuit.com/circuit/5611864725782528/passive-rfid}{\alert{\underline{Lien vers le circuit d'exemple}}}}
	\begin{itemize}
		\item Où est l'émetteur ? 
		\item Où est le tag ?
		\item Quel est le signal qui devrait être interprété par le lecteur ?
		\item Quelle est la fréquence de résonance du tag ? 	
		\item Quel composant contient le code du tag ?
		\item Quelle partie du tag est son auto-alimentation?
		\item A quoi servent les transistors ?
		\item Quel est l'impact du coefficient de couplage ?  
	\end{itemize}
\end{block}
...
\begin{frame}{Principe}
	\centering
	\includegraphics[width=0.8\linewidth]{principe_3phase}
\end{frame}


\begin{frame}{Circuit constitué de trois circuits appelés {\em phases} $a$, $b$ et $c$ }
	\centering
	\includegraphics[width=0.8\linewidth]{principe_3phase_phases}
\end{frame}

\begin{frame}{Production $\rightarrow$ transport $\rightarrow$ consommation} 
	\begin{center}
		\includegraphics[width=0.8\linewidth]{principe_3phase_production_transport_charge.pdf}
	\end{center}
\end{frame}
\begin{frame}{Production $\rightarrow$ transport $\rightarrow$ consommation} 
	\begin{itemize}
		\item \textbf{générateur} : circuit à 3 accès qui délivre 3 f.e.m, équilibré si
		...
		\item \textbf{charge} : circuit à 3 accès, équilibré si chaque phase est
		constituée d'une impédance identique;
		\item \textbf{circuit de transmission} : relie la charge au générateur,
		équilibré si chaque phase possède une impédance identique.
	\end{itemize}

	Un système est dit \textbf{équilibré} si tous ses constituants le sont.
\end{frame}
...
\begin{frame}[allowframebreaks]{Tensions et courants triphasés équilibrés}
...
\begin{frame}{Montage en étoile}
  ...
  \begin{column}{0.6\textwidth}
	\begin{center}
	\includegraphics[width=\linewidth]{principe_3phase}
	\end{center}
  \end{column}
...
\begin{itemize}
	\item tension de phase : 230 V (tension phase-neutre)
	\item tension de ligne : 400 V
\end{itemize}
\end{frame}
...
\begin{frame}{Mise à la terre des  neutres}
  ...
  \begin{column}{0.35\textwidth}
	...
	\textbf{En pratique}, il existe toujours un léger déséquilibre entre les
	phases et \textbf{on connecte les neutres à la terre}. 
  \end{column}
  \begin{column}{0.6\textwidth}
	\begin{center}
	\includegraphics[width=\linewidth]{3phase_grounding}	
	\end{center}
  \end{column}
\end{columns}
...
\begin{frame}{Si tout est équilibré}
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{eqnarray*}
				\bar{I}_{a}& =&
...
\begin{frame}[allowframebreaks]{Montage étoile}
...
\begin{frame}[allowframebreaks]{Montage triangle}
